...
save_dir = "model_logs/atomic/"
if not os.path.isdir(save_dir): 
    os.mkdir(save_dir)
logging.basicConfig(filename=f'{save_dir}log_ckb.log',filemode='w',level=logging.INFO)
...
def main(num):
    ...
    logger.info(data_loader.sequences["train"]["total"].size(0))
    data_loader.opt = opt
    ...
    logger.info(data_loader.__dict__.keys())
    ...
    model = models.make_model(
        ...
        #    load=False)
        #Gengyu: train on triplets from Connected KB
        load=(opt.net.init=="pt"))
    #model_file = "pretrained_models/atomic_pretrained_model.pickle"
    #_, state_dict = interactive.load_model_file(model_file)
    #models.load_state_dict(model, state_dict)

    logger.info("Done.")

    logger.info("Files will be logged at: {}".format(
    ...
    if config.gpu_mode:
        logger.info("Pushing to GPU: {}".format(config.gpu_index))
